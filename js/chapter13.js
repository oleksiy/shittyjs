window.onload = () => {
    ...
    let showRandomLyric = function (e){
        console.log("EVENT CAME FROM" + e.target);
        let lyricsSpace = document.getElementById("lyrics");
        let lyricsArray = sickoMode.toString().split('\n');
        console.log(`Total number of available lyrics ${lyricsArray.length}`);
        let verseOne = getRandomNumber(lyricsArray.length-1), verseTwo = getRandomNumber(lyricsArray.length-1), verseThree = getRandomNumber(lyricsArray.length-1);
        let verses = [verseOne, verseTwo, verseThree];
        console.log("Verse array: " + verses.toString());
        for(let i = 0; i < 3; i++ ){
            let textNode =  document.createTextNode(lyricsArray[verses[i]]);
            lyricsSpace.appendChild(textNode);
            lyricsSpace.appendChild(document.createElement("br"));
        }
    }
    ...
    button.setAttribute("value", "New Lyric");
    button.setAttribute("type", "button");
    button.addEventListener("click", showRandomLyric);
